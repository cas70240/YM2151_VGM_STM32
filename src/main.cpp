...
//Sound Chips
const int prev_btn = PA2;
const int rand_btn = PA1;
const int next_btn = PA0;
const int loop_btn = PC15;
const int shuf_btn = PC14;
//                    D0    D1    D2 ・・・
int YM_Datapins[8] = {PB10, PB11, PB12, PB13, PB14, PB15, PA8, PA9};

const int YM_CS = PA15;
const int YM_RD = PB3;
const int YM_WR = PB4;
const int YM_A0 = PB5;
const int YM_IC = PB8; 
...
HardwareTimer *pTimer4 = new HardwareTimer(TIM4);
...
{
  pTimer4->pause();
  pTimer4->setPrescaleFactor(1);
//pTimer4->setOverflow(1633);  // 値を小さくすると速くなる
  pTimer4->setOverflow(1400);
  //pTimer4->setChannel1Mode(TIMER_OUTPUT_COMPARE);
  //pTimer4->attachCompare1Interrupt(tick);
  pTimer4->setMode(1, TIMER_OUTPUT_COMPARE);
  pTimer4->attachInterrupt(tick);
  pTimer4->refresh();
  pTimer4->resume();  
}
